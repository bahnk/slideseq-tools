...
class Molecule
{
	...
	public:
		...
		std::map<unsigned long long, std::tuple<std::string, std::string>> GetRecordTags() const;
		void ComputeFrequencies();
		std::string GetFrequenciesString() const;
		long long GetMaxFrequency() const;
		bool IsThereAMajority() const;
		std::map<unsigned long long, std::tuple<std::string, std::string>> GetFrequencyBasedRecordTags();
